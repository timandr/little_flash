...
const char* lfs_test_hello_str = "Hello, World! new string\n";
...
void test_lfs_overwrite_append(const char* filename)
{
    ...
    test_lfs_create_file_with_text(filename, "aaaa");
    ...
    test_lfs_create_file_with_text(filename, "cccc");
...
void test_lfs_read_file(const char* filename)
{
    ...
    TEST_ASSERT_EQUAL(strlen(lfs_test_hello_str), cb);
    TEST_ASSERT_EQUAL(0, strcmp(lfs_test_hello_str, buf));
    TEST_ASSERT_EQUAL(0, fclose(f));
}
...
{
    test_lfs_create_file_with_text(filename, "foo\n");
...
{
    test_lfs_create_file_with_text(filename, "unlink\n");
...
void test_lfs_mkdir_rmdir(const char* filename_prefix)
{
    ...
    test_lfs_create_file_with_text(name_dir2_file, "foo\n");
...
void test_lfs_can_opendir(const char* path)
{
    ...
    test_lfs_create_file_with_text(name_dir_file, "test_opendir\n");
...
void test_lfs_opendir_readdir_rewinddir(const char* dir_prefix)
{
    ...
    test_lfs_create_file_with_text(name_dir_file1, "1\n");
    test_lfs_create_file_with_text(name_dir_file2, "2\n");
    test_lfs_create_file_with_text(name_dir_file3, "\01\02\03");
    TEST_ASSERT_EQUAL(0, mkdir(name_dir_inner, 0755));
    test_lfs_create_file_with_text(name_dir_inner_file, "3\n");
    ...
    while(count < 6) {
        ...
        printf("found '%s'\n", de->d_name);
        if (strcasecmp(de->d_name, ".") == 0) {
            TEST_ASSERT_TRUE(de->d_type == DT_DIR);
            names[count] = ".";
            ++count;
        } else if (strcasecmp(de->d_name, "..") == 0) {
            TEST_ASSERT_TRUE(de->d_type == DT_DIR);
            names[count] = "..";
            ++count;
        } else if (strcasecmp(de->d_name, "1.txt") == 0) {
    ...
    TEST_ASSERT_EQUAL(count, 6);

    rewinddir(dir);
    TEST_ASSERT_EQUAL(0, telldir(dir));
    struct dirent* de = readdir(dir);
    TEST_ASSERT_NOT_NULL(de);
    TEST_ASSERT_EQUAL(0, strcasecmp(de->d_name, names[0]));
    TEST_ASSERT_EQUAL(1, telldir(dir));
    seekdir(dir, 3);
    TEST_ASSERT_EQUAL(3, telldir(dir));
    de = readdir(dir);
    TEST_ASSERT_NOT_NULL(de);
    TEST_ASSERT_EQUAL(0, strcasecmp(de->d_name, names[3]));
    TEST_ASSERT_EQUAL(4, telldir(dir));
    seekdir(dir, 1);
    TEST_ASSERT_EQUAL(1, telldir(dir));
    de = readdir(dir);
    TEST_ASSERT_NOT_NULL(de);
    TEST_ASSERT_EQUAL(0, strcasecmp(de->d_name, names[1]));
    TEST_ASSERT_EQUAL(2, telldir(dir));
    seekdir(dir, 2);
    TEST_ASSERT_EQUAL(2, telldir(dir));
    de = readdir(dir);
    TEST_ASSERT_NOT_NULL(de);
