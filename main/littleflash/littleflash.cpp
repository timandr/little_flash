...
esp_err_t LittleFlash::init(const little_flash_config_t *config)
{
    ...
    /* Configuration for external flash*/
    if (cfg.flash)
    {
    ...
    /* Configuration for internal flash*/
    else
    {
    ...
    /* Details description of configuration there is in header lfs.h */
    lfs_cfg.context     = (void *) this;
    lfs_cfg.read_size   = EXT_FLASH_PAGE_SIZE;
    lfs_cfg.prog_size   = EXT_FLASH_PAGE_SIZE;
    ...
    /* Probably this first initialization. Checking... */
    if (err < 0)
    {
        lfs_unmount(&lfs);
        if (!cfg.auto_format)
        {
        	ESP_LOGE(TAG, "%s: lfs_mount error. !cfg.auto_format", __func__);
            return ESP_FAIL;
        }
        ...
        {
            lfs_unmount(&lfs);
            ESP_LOGE(TAG, "%s: lfs_format error. ret: %d", __func__, err);
            return ESP_FAIL;
        }
        ...
        {
            lfs_unmount(&lfs);
            ESP_LOGE(TAG, "%s: lfs_mount error. twice", __func__);
            return ESP_FAIL;
        }
    }
    mounted = true;

    /* Allocate memory for a file descriptors */
    ...
    /* Connecting the VFS from ESP-IDF */
    esp_vfs_t vfs = {};
